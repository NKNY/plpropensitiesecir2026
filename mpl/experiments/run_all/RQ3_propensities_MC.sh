...
for ds in $datasets; do
  for M in $Ms; do
    if [[ $M -eq 10000001 ]]; then seed=1; else seed=0; fi
    for policy in $policies; do
      for fold in $folds; do
        for subset in $subsets; do
          echo "python mpl/experiments/propensities.py --params_path configs/propensities/propensities_RQ3_${ds}_${policy}.json --fold ${fold} --subset ${subset} --N ${M} --K 10 --seed ${seed} --method MC" >> ${output_filename}
        done
      done
    done
  done
