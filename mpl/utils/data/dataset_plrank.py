...
class DataFoldSplit(object):
    ...
    def __iter__(self):
        if self.iterable_idx < self.num_queries():
            self.iterable_idx += 1
            yield (
                self.iterable_idx - 1,
                self.query_feat(self.iterable_idx - 1),
                self.query_labels(self.iterable_idx - 1),
            )
        else:
            raise StopIteration
...
class DataFold(object):
    ...
    def read_data(self):
        ...
        if not data_read:
            feature_map = {}
            (train_doclist_ranges, train_feature_matrix, train_label_vector) = (
                self._read_file(train_raw_path, feature_map, False)
            )
            (valid_doclist_ranges, valid_feature_matrix, valid_label_vector) = (
                self._read_file(valid_raw_path, feature_map, False)
            )
            (test_doclist_ranges, test_feature_matrix, test_label_vector) = (
